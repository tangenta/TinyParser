...
class AlgorithmTest extends BasicSpec {
  ...
  "Follow() or productions" should "be correct" in {
    ...
    assertResult(result1) {testPFollow(NonTerminal("T"))}
    assertResult(result1) {testPFollow(NonTerminal("Tp"))}
    assertResult(result2) {testPFollow(NonTerminal("E"))}
    assertResult(result2) {testPFollow(NonTerminal("Ep"))}
    assertResult(result3) {testPFollow(NonTerminal("F"))}
  }
